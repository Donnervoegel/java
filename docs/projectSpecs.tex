...
\section{Test Suite}
The test suite is the section of the grading system that allows the marker to evaluate code.
\subsubsection{Functionality}
The submitted code will be view-able side by side with the solution code.  The code will be compiled on the server in an insulated environment.  Only Python will be implemented for the prototype.  The test suite will also allow a section for the marker to view the output of submitted code.
\subsubsection{Format}
The test suite will fit into the main content area of the grading system.  The top 2/3 of the content area will be divided into two sections.  The left will contain the correct or solution code while the right will contain the student's submitted work.  The answer half of the content area can be switched to the rubric(and subsequently back to the solution code) if the marker desires.  The bottom 1/3 of the content area will contain a console window with buttons to run and stop the submitted code.
